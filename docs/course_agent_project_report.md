...
```mermaid
flowchart TD
    UI["Gradio 前端演示界面"] --> API["FastAPI REST / WebSocket API"]
...
前端演示使用 `Gradio`，并通过 `httpx` 调用 FastAPI 后端，而不是直接调用 Python 内部对象。这样可以保持前后端解耦。
...
项目当前使用 Gradio 作为演示前端。用户可以在页面中选择练习模式或专业面试模式，输入 JD，上传简历，开始面试，输入回答，并查看最终报告。

前端的特点包括：

- 通过 `httpx` 调用 FastAPI 后端，实现前后端解耦。
- 支持展示对话历史和当前面试状态。
- 支持练习报告和专业双轮面试报告。
- 使用雷达图展示技能评分。
...
第四，前端目前使用 Gradio，适合课程展示和快速演示。如果要进一步产品化，可以使用 React 或 Next.js 构建更完整的前端体验，包括用户登录、历史报告管理、题目复盘和学习路径。
